export const submitOrder = async (formData) => {
  ...
  try {
    ...
    const orderResponse = await fetch(
      'http://192.168.1.39:3000/api/order/pooja-booking',
      {
        method: 'POST',
        headers: {
    ...
    const addressResponse = await fetch(
      'http://192.168.1.39:3000/api/order/delivery-address',
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
    ...
    if (!addressResponse.ok) {
      return {
        success: false,
        message:
          addressDataResponse.message || 'Failed to save delivery address',
      };
    }
    const cartClearResponse = await fetch(
      `http://192.168.1.39:3000/api/cart/remove/${formData.cartId}`,
      {
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${token}`,
    ...
    if (!cartClearResponse.ok) {
      return {
        success: false,
        message: cartClearData.message || 'Failed to clear cart',
      };
    }
...
export const submitBhajanBooking = async (formData) => {
  ...
  try {
    ...
    // Step 1: Submit Order
    const orderResponse = await fetch(
      'http://192.168.1.39:3000/api/order/bhajan-mandali',
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
