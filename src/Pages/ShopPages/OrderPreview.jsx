  ...
  const fetchData = async () => {
    try {
        ...
        const [orderResponse, addressResponse] = await Promise.all([
            fetch(`http://34.131.41.101:3000/api/orders/${id}`, {
                method: "GET",
                headers: {
        ...
        if (orderData.bookingDetails?.Type === "Bhajan Mandali") {
            const mandaliResponse = await fetch(`http://34.131.41.101:3000/api/bhajanMandal/single_bhajan/${orderData.bookingDetails.mandaliId}`, {
                method: "GET",
                headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${token}`,
...
useEffect(() => {
  ...
  const handleConfirmOrder = async () => {
    ...
    try {
        ...
        if (orderType === "Pooja") {
            updateUrl = "http://34.131.41.101:3000/api/orders/update-order";
            orderDetails = {
                bookingId: id,
            ...
            updateUrl = "http://34.131.41.101:3000/api/order/update-mandali-order";
            orderDetails = {
                bookingId: id,
